...
export class ImageProvider {
    ...
    /**
     * Helper to get buffer from either a data URL, HTTP URL, or file path
     * Supports:
     * - data:image/png;base64,... (data URLs)
     * - http://... or https://... (HTTP URLs)
    ...
    async getImageBuffer(input) {
        ...
        if (input.startsWith('http://') || input.startsWith('https://')) {
            logger.info(`Fetching image from HTTP URL: ${input}`);
            try {
                const response = await fetch(input);
                if (!response.ok) {
                    throw new ProviderError(`Failed to fetch image from URL: ${response.status} ${response.statusText}`, this.name, true);
                }
                ...
                throw new ProviderError(`Failed to fetch image from URL: ${input}. Error: ${error instanceof Error ? error.message : 'Unknown error'}`, this.name, true);
            }
        }
