...
import { GenerateInputSchema, EditInputSchema, ProviderError, ResponseFormat, ProviderName } from './types.js';
...
/**
 * Load image data from various sources: data URL, file path, or HTTP URL
 * Returns a Buffer containing the image data
 */
async function loadImageFromSource(source: string): Promise<Buffer> {
  // Data URL (base64)
  ...
  if (source.startsWith('http://') || source.startsWith('https://')) {
    logger.info(`Fetching image from HTTP URL: ${source}`);
  ...
  logger.info(`Loading image from file path: ${filePath}`);
  return await fs.readFile(filePath);
}

/**
 * Build MCP response content array with text and inline images via HTTP URLs
 * Images are served via IMAGE_SERVER_URL environment variable
 */
function buildResponseContent(
  responseText: string,
  _images: Array<{ dataUrl: string; format?: string }>,
  savedPaths: Array<{ path: string; format: string; size: number }>
): Array<{ type: string; text?: string }> {
  const content: Array<{ type: string; text?: string }> = [];

  // Get base URL for serving images (defaults to localhost:6002)
  const imageServerUrl = process.env.IMAGE_SERVER_URL || 'http://192.168.1.201:6002';

  // Build markdown with HTTP URLs for images
  let markdownText = responseText + '\n\n';

  for (let i = 0; i < savedPaths.length; i++) {
    const filename = savedPaths[i].path.split('/').pop();
    const imageUrl = `${imageServerUrl}/${filename}`;
    markdownText += `![Generated Image ${i + 1}](${imageUrl})\n\n`;
  }

  // Add text content with metadata and image URLs
  content.push({
    type: 'text',
    text: markdownText
  });
 ...
 */
function truncateIfNeeded(text: string, context: string): string {
...
function formatGenerateResultMarkdown(data: {
  images: Array<{ path: string; url?: string; format: string; size: number }>;
  provider: string;
  model?: string;
  warnings?: string[];
  note?: string;
}): string {
  const lines: string[] = ['# Image Generation Result', ''];
  ...
  data.images.forEach((img, idx) => {
    ...
    if (img.url) {
      lines.push(`- **URL** (for editing): \`${img.url}\``);
    }
...
function formatEditResultMarkdown(data: {
  ...
  const lines: string[] = ['# Image Edit Result', ''];
  ...
  data.images.forEach((img, idx) => {
    lines.push(`### Image ${idx + 1}`);
    if (img.url) {
      lines.push(`- **URL** (for further editing): \`${img.url}\``);
    }
    lines.push(`- **Path**: \`${img.path}\``);
    lines.push(`- **Format**: ${img.format}`);
    lines.push(`- **Size**: ${(img.size / 1024).toFixed(2)} KB`);
    ...
    tools: [
      ...
      },
      {
        ...
        description: 'Edit existing images with text prompts using AI image editing providers.\n\n' +
          ...
          '      Formats: http://... OR https://... OR data:image/png;base64,... OR /path/to/image.png OR file:///path/to/image.png\n' +
          '      Note: HTTP URLs from previous image_generate responses can be used directly\n' +
          '  - maskImage (string, optional): Mask indicating edit regions (white=edit, black=preserve)\n' +
          '      Formats: Same as baseImage (HTTP URLs, data URLs, or file paths). Not required for all providers/operations\n' +
          '  - provider (string, optional): Provider name or "auto" (default)\n' +
  ...
  try {
    switch (name) {
      ...
      case 'image_generate': {
        ...
        try {
          ...
          const responseData = {
            ...
            note: 'Images saved to disk. Use the "url" field for image_edit baseImage parameter.'
          };
          ...
          responseText = truncateIfNeeded(
            responseText,
            'Tip: Response was truncated due to size. Images are saved to disk and accessible via file paths.'
          );

          // Return text with markdown image URLs for inline display
          return {
            content: buildResponseContent(responseText, result.images, savedImages)
          };
        } catch (error) {
          if (error instanceof ProviderError && error.isRetryable && process.env.DISABLE_FALLBACK !== 'true') {
            ...
            if (fallback.name !== provider.name) {
              ...
              const responseData = {
                ...
                note: 'Images saved to disk. Use the "url" field for image_edit baseImage parameter. Fallback provider was used.'
              };
              ...
              responseText = truncateIfNeeded(
                responseText,
                'Tip: Response was truncated. Images are saved to disk. Fallback provider was used.'
              );
      ...
      case 'image_edit': {
        ...
        // For auto-selection, prefer BFL for edits as it has the best inpainting quality
        let provider;
        if (input.provider === 'auto' || !input.provider) {
          ...
          // Check if this is an SVG/vector edit - Recraft is the only provider that supports SVG
          const promptLower = input.prompt.toLowerCase();
          const isSvgRequest = promptLower.includes('svg') || promptLower.includes('vector');

          let selectedName: ProviderName | undefined;

          if (isSvgRequest && editCapableProviders.includes('RECRAFT')) {
            // Route SVG/vector edits to Recraft - it's the only provider with SVG support
            selectedName = 'RECRAFT';
            logger.info('Auto-selected RECRAFT for SVG/vector editing (only provider with SVG support)');
          } else {
            // Prefer BFL for edits - it has the best inpainting/editing quality with flux-kontext-pro
            // Fall back to OpenAI, then others if BFL is not available
            const preferredEditOrder: ProviderName[] = ['BFL', 'OPENAI', 'STABILITY', 'GEMINI', 'CLIPDROP', 'IDEOGRAM'];
            for (const preferred of preferredEditOrder) {
              if (editCapableProviders.includes(preferred)) {
                selectedName = preferred;
                logger.info(`Auto-selected ${preferred} for image editing (preferred for quality)`);
                break;
              }
            }
          }

          if (!selectedName && editCapableProviders.length > 0) {
            selectedName = editCapableProviders[0] as ProviderName;
          }
          provider = selectedName ? Config.getProvider(selectedName) : Config.getProviderWithFallback(undefined, input.prompt);
        } else {
          provider = Config.getProviderWithFallback(input.provider, input.prompt);
        ...
        if (provider.name === 'BFL' && (input.provider === 'auto' || !input.provider)) {
          ...
          const imageBuffer = await loadImageFromSource(input.baseImage);
          ...
          if (Math.abs(ratio - 1) > 0.05) {
            ...
            // Get alternative edit providers excluding BFL, respecting our preference order
            const alternatives = Config.getConfiguredEditProviders().filter(name => name !== 'BFL');
            if (alternatives.length > 0) {
              // Use same preference order as above, but without BFL
              const preferredNonBfl = ['OPENAI', 'STABILITY', 'GEMINI', 'CLIPDROP', 'IDEOGRAM'];
              let fallbackName: ProviderName | undefined;
              for (const preferred of preferredNonBfl) {
                // Cast alternatives to string[] for comparison since TS narrows the type after filter
                if ((alternatives as string[]).includes(preferred)) {
                  fallbackName = preferred as ProviderName;
                  break;
                }
              }
              if (!fallbackName) {
                fallbackName = alternatives[0];
              }
              provider = Config.getProvider(fallbackName)!;
        ...
        const responseData = {
          ...
          note: 'Edited images saved. Use the "url" field for further image_edit operations.'
        };
        ...
        responseText = truncateIfNeeded(
          responseText,
          'Tip: Response was truncated due to size. Edited images are saved to disk and accessible via file paths.'
        );
